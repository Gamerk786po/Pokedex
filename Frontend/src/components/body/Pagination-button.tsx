...
const PaginationButton: React.FC<PaginationButtonProps> = ({label, onClick, disabled}) => {
    return(
        <div>
            ...
            <span className="absolute left-0 top-0 w-full h-full bg-[#001f3f] transform translate-y-full group-hover:translate-y-5 transition-transform duration-300 ease-in-out z-0 rounded-t-[20%]"></span>
            </button>
        </div>
    )
}
